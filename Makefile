...
.PHONY: lint lint-python validate-static validate-k8s test test-flink test-api test-python
...
dlq-summary:
	$(API_CURL) "http://localhost:8000/dlq/summary?limit=100&from_beginning=true" | python3 -m json.tool

dlq-replay-preview:
	$(API_CURL) -X POST "http://localhost:8000/dlq/replay" -H "content-type: application/json" -d '{"max_messages":5,"scan_limit":100,"dry_run":true}' | python3 -m json.tool
...
validate-k8s:
	@find k8s -type f -name kustomization.yaml -exec dirname {} \; | sort -u | while IFS= read -r directory; do \
...
test-api:
	docker build --target test -t $(PROJECT_NAME)-api-test -f api/Dockerfile .

test-python:
	docker build --target test -t $(PROJECT_NAME)-python-test -f Dockerfile.python-quality .
...
clean:
	...
	docker image rm $(PROJECT_NAME)-python-lint 2>/dev/null || true
	docker image rm $(PROJECT_NAME)-python-test 2>/dev/null || true
